 ...
 * {name}[ TIPO ]
 * Descricao
 * @copyright (c) year, Alessandro da Rocha Filho
 */
class Pager {
     ...
     */
    function __construct($Link, $First = null, $Last = null, $MaxLinks = null, $Href = true) {
        $this->Link = (string) $Link;
        $this->Href = (boolean) $Href;
     ...
     */
    public function ExePaginator($Tabela = null, $Termos = null, $ParseString = null, $FullQuery = null) {
        $this->Tabela = (string) $Tabela;
        $this->Termos = (string) $Termos;
        $this->Places = (string) $ParseString;
        $this->FullQuery = (string) $FullQuery;
        $this->getSyntax();
    }
    ...
    private function getSyntax() {
        ...
        if ($this->Rows > $this->Limit):
            ...
            $this->Paginator = "<nav aria-label='...'><ul class='pagination justify-content-end mb-0'>";
            $this->Paginator .= "<li class='page-item'>
                <a title='{$this->First}' class='page-link ".($this->Href ? "" : "j_btn_pag")."' pag='1' href='{$this->Link}1'>
                    <i class='fas fa-angle-left'></i>
                    <span class='sr-only'>{$this->First}</span>
                </a>
            </li>";
            ...
            for ($iPag = $this->Page - $MaxLinks; $iPag <= $this->Page - 1; $iPag ++):
                if ($iPag >= 1):
                    $this->Paginator .= "<li class='page-item'>
                        <a title='Página {$iPag}' class='page-link ".($this->Href ? "" : "j_btn_pag")."' pag='{$iPag}' href='{$this->Link}{$iPag}'>{$iPag}</a>
                    </li>";
                endif;
            endfor;

            $this->Paginator .= "<li class='page-item active'>
                <a class='page-link' href='#'>{$this->Page}</a>
            </li>";

            for ($dPag = $this->Page + 1; $dPag <= $this->Page + $MaxLinks; $dPag ++):
                if ($dPag <= $Paginas):
                    $this->Paginator .= "<li class='page-item'><a title='Página {$dPag}' class='page-link ".($this->Href ? "" : "j_btn_pag")."' pag='{$dPag}' href='{$this->Link}{$dPag}'>{$dPag}</a></li>";
                endif;
            endfor;

            $this->Paginator .= "<li class='page-item'>
                <a title='{$this->Last}' class='page-link ".($this->Href ? "" : "j_btn_pag")."' pag='{$Paginas}' href='{$this->Link}{$Paginas}'>
                    <i class='fas fa-angle-right'></i>
                    <span class='sr-only'>{$this->Last}</span>
                </a>
            </li>";
